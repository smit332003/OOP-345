...
void movieAddedObserver(const Collection<Movie>& theCollection,
   ...
   std::cout << "Movie \"" << theMovie.title() << "\" added to collection \""
             << theCollection.name() << "\" (" << theCollection.size()
             << " items).\n";
}
...
int main(int argc, char** argv) {
   ...
   if (argc == 5) {
      ...
      //       - store each book read into the collection "library" (use the +=
      //       operator)
   ...
   // TODO : (from part #1)create a lambda expression that fixes the price of a
   // book accoding to the rules
   //       - the expression should receive a single parameter of
   //       type "Book&"
   //       - if the book was published in US, multiply the price
   //       with "usdToCadRate"
   //            and save the new price in the book object
   //       - if the book was published in UK between 1990 and 1999
   //       (inclussive),
   //            multiply the price with "gbpToCadRate" and save
   //            the new price in the book object
   ...
   for (size_t i = 0; i < library.size(); i++) {
      fixSpelling(library[i]);
   }

   // TODO (from part #1): iterate over the library and update the price of each
   // book
